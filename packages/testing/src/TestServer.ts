 ...
 * Requires environment variables from .env.test
 */
...
// Resolve .env.test from packages/backend/ — that's where the test env config lives.
// packages/testing/src/ → ../../backend/ = packages/backend/
const envPath = join(__dirname, '..', '..', 'backend', '.env.test');
...
import { AuthHandler, type AuthResult } from '../../backend/src/handlers/auth-handler';
import { EventHandler } from '../../backend/src/handlers/event-handler';
import { WebSocketHandler } from '../../backend/src/handlers/websocket-handler';
import { AutoplayService } from '../../backend/src/services/autoplay-service';
import { BoardService } from '../../backend/src/services/board-service';
import { ChannelManager } from '../../backend/src/services/channel-manager';
import { McaService } from '../../backend/src/services/mca-service';
import { PubSubService } from '../../backend/src/services/pubsub-service';
import { SessionManager } from '../../backend/src/services/session-manager';
import { VolumeService } from '../../backend/src/services/volume-service';
import { WorkspaceService } from '../../backend/src/services/workspace-service';
import type { McaManager } from '../../backend/src/services/mca-manager';
import { MongoSessionStore } from '../../backend/src/session/MongoSessionStore';
...
export interface TestServerOptions {
  ...
  /**
   * Wire the full domain stack (PubSubService + WorkspaceService + BoardService
   * + McaService + AutoplayService + VolumeService + EventHandler). Required to
   * exercise WsFramework actions like agent.create / workspace.create /
   * board.create-project / app.install and to receive their broadcast events.
   * Off by default to keep existing tests minimal.
   */
  enableNavbarStack?: boolean;

  /**
   * Inject an McaManager mock for domains that depend on it (e.g. feedback
   * report action). Avoids starting the real Docker-backed manager in tests.
   */
...
class MockAuthHandler extends AuthHandler {
  async authenticate(message: any): Promise<AuthResult> {
    ...
    if (message.method === 'credentials') {
      ...
      // Use "user:" prefix so tests can match /^user:/
      const userId = `user:test_${email.split('@')[0]}` as any;
      return {
        success: true,
        userId,
    ...
    // Token auth — only accept tokens that were issued by this mock handler
    if (message.method === 'token' && message.sessionToken) {
      if (!message.sessionToken.startsWith('token_')) {
...
export interface TestServerInstance {
  ...
  // Optional services — only set when enableNavbarStack: true
...
export async function createTestServer(
  ...
  // Optional full domain stack — required to exercise agent/workspace/board/app
  // WsFramework actions and to receive broadcast events (TER-304).
  let pubSubService: PubSubService | undefined;
  let workspaceService: WorkspaceService | undefined;
  let boardService: BoardService | undefined;
  let mcaService: McaService | undefined;
  let volumeService: VolumeService | undefined;
  let autoplayService: AutoplayService | undefined;
  let eventHandler: EventHandler | undefined;

  if (options.enableNavbarStack) {
    // Volumes go to a temp dir so tests don't leak filesystem state.
    const volumesBase = mkdtempSync(`${tmpdir()}/teros-test-volumes-`);
    volumeService = new VolumeService(db, { basePath: volumesBase });

    workspaceService = new WorkspaceService(db, volumeService);
    await workspaceService.ensureIndexes();

    boardService = new BoardService(db);
    await boardService.ensureIndexes();

    mcaService = new McaService(db, { workspaceService, volumeService });
    await mcaService.ensureIndexes();

    eventHandler = new EventHandler(db, sessionManager, channelManager);
    ...
    autoplayService = new AutoplayService(db, pubSubService, channelManager);
  ...
  return {
    ...
    async seedAgents(): Promise<void> {
      // Create cores first — the two internal cores of the consolidated model
      // (matches production: agents are mapped by scope, never to a custom core).
      ...
      const cores = [
        { coreId: 'agent', coreType: 'agent', name: 'Agent', fullName: 'Teros Agent', ...baseCore },
        {
          coreId: 'super-agent',
      ...
      // Seed test users with accessGranted: true so UserService.hasAccess() passes.
      // The MockAuthHandler generates userId = "user:test_<localPart>", so we seed
      // the two most common test identities here.
      const testUsers = [
        {
          userId: 'user:test_test',
          email: 'test@example.com',
          accessGranted: true,
          emailVerified: true,
          status: 'active',
        ...
        {
          userId: 'user:test_pablo',
          email: 'pablo@teros.ai',
          accessGranted: true,
          emailVerified: true,
          status: 'active',
          profile: { name: 'Pablo', avatarUrl: null },
          createdAt: new Date(),
          updatedAt: new Date(),
        },
        {
          ...
          emailVerified: true,
          status: 'active',
          profile: { name: 'User 1', avatarUrl: null },
          createdAt: new Date(),
          updatedAt: new Date(),
        },
        {
          userId: 'user:test_user2',
          email: 'user2@test.com',
          accessGranted: true,
          emailVerified: true,
          status: 'active',
          profile: { name: 'User 2', avatarUrl: null },
          createdAt: new Date(),
          updatedAt: new Date(),
        },
      ];

      // Create agents — ownerId must match a test user so agent.list returns them
      const agents = [
        {
          agentId: 'agent:iria',
          coreId: 'super-agent',
          ownerId: 'user:test_test',
          workspaceId: null,
          name: 'Iria',
          fullName: 'Iria Devon',
          role: 'AI Assistant',
