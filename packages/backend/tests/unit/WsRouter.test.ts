...
describe('WsRouter', () => {
  ...
  describe('register', () => {
    ...
    it('should throw if registering a duplicate action', () => {
      router.register('profile.get', mock());
      expect(() => router.register('profile.get', mock())).toThrow(
        'Handler already registered for action: profile.get',
      );
    });

    it('should list all registered actions sorted', () => {
      router.register('channels.create', mock());
      router.register('agents.list', mock());
      router.register('profile.get', mock());
  ...
  describe('dispatch', () => {
    ...
    it('should pass context and data to the handler', async () => {
      const ws = createMockWs();
      const handler = mock(async (ctx: any, data: any) => ({ received: data }));
    ...
    it('should send error when handler throws a generic Error', async () => {
      ...
      // TER-397: un Error crudo (sin userMessage) se normaliza a un mensaje
      // user-facing; el técnico ('Something went wrong') se oculta al cliente.
      expect(msg.message).toBe('Ocurrió un error inesperado. Intenta de nuevo.');
    });
