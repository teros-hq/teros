/**
 * Google Docs API Client Manager
 *
 * Manages OAuth2 authentication and Google Docs API client.
 */
...
export async function initializeGoogleClients(context: ToolContext): Promise<GoogleClients> {
  ...
  if (!clientId || !clientSecret) {
    throw new Error(
      'Google Docs OAuth credentials not found.\n' +
...
export async function withAuthRetry<T>(
  ...
  try {
    ...
    console.error(`[GoogleDocs] 401 error in ${operationName}, attempting refresh...`);
