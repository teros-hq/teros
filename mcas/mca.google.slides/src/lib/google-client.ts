 ...
 *
 * Manages OAuth2 authentication and Google Slides API client.
 */
...
export async function initializeGoogleClients(context: ToolContext): Promise<GoogleClients> {
  ...
  if (!clientId || !clientSecret) {
    throw new Error(
      'Google Slides OAuth credentials not found.\n' +
...
export async function withAuthRetry<T>(
  ...
  try {
    ...
    console.error(`[GoogleSlides] 401 error in ${operationName}, attempting refresh...`);
